[Code]
//  Contains source code from LEGO Island Alternate Installer
...
function FindUniqueFile( filename: AnsiString ): Ansistring;
var
	i: integer;
begin
	for i:=0 to GetArrayLength(DrvLetters) -1 do
	begin
		if FileExists( DrvLetters[i] + filename) then
		begin
			result:=DrvLetters[i];
			exit;
		end
...
begin
	...
	begin
		MsgBox('No disc drive was detected in your computer.', mbError, MB_OK);
		Abort;
	end;

  while FindUniqueFile('TonicTrouble.exe') = '' do
  begin
    if (FindUniqueFile('MaiD3Dvr.exe') <> '') or (FindUniqueFile('MAIDFXVD.EXE') <> '') then
      mbText:='You have inserted a Tonic Trouble Special Edition disc.'#13#10'Tonic Trouble Fix is only compatible with the retail release of Tonic Trouble.'#13#10#13#10'Please insert a retail Tonic Trouble disc to continue.'
    else
      mbText:='No Tonic Trouble disc was detected.'#13#10'Please insert a Tonic Trouble disc to continue.';
    if MsgBox(mbText, mbError, MB_RETRYCANCEL or MB_DEFBUTTON1) = IDRETRY then

    else
      Abort;
  end;

	Result:=FindUniqueFile('TonicTrouble.exe');
end;